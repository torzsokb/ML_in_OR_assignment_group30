...
def inner_cv(outer_fold: int, alpha: float, l1_ratio: float) -> float:
    inner_mse = []
...
def objective(trial):
    ...
    return inner_cv(outer_fold=current_fold, alpha=alpha, l1_ratio=l1_ratio)
...
if __name__ == "__main__":
    ...
    for current_fold in range(n_folds):
        ...
        print(f"Fold {current_fold} HOLDOUT MSE: {mse:.4f}")
        results.append(mse)
