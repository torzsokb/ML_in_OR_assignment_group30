...
for fold in range(n_folds):
    ...
    outer_train_data = xgb.DMatrix(data = train_df.drop(["y2", "cv_fold"], axis=1), label=train_df["y2"])
    ...
    for i, inner_fold in enumerate(fold_numbers):
        if not inner_fold == fold:
            ...
            inner_train_data = xgb.DMatrix(data = inner_train_df.drop(["y2", "cv_fold"], axis=1), label=inner_train_df["y2"])
...
def main():
    ...
    output = {"cv-rmse": [], "train-rmse": [], "oos-rmse": [], "train-poisson-nloglik": [], "oos-poisson-nloglik": []}
    fold_params = {}
    feature_importances = {}
